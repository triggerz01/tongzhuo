...
```bash
git clone --depth 1 https://github.com/triggerz01/tongzhuo.git
cd tongzhuo
```

`--depth 1` 只拉最新一版的文件树，**不下载历史里的旧版本**。
早期提交里塞过 30MB 的模型和动作文件，不加这个参数会把它们一起拖下来，
国内的速度下能等到你怀疑人生。加了之后克隆内容只有 1.7MB。

浅克隆之后 `git pull` / `git push` 照常用，不影响任何操作。

> **大文件不在仓库里**，通过微信/网盘单独发。拿到素材包后按
> [assets/README.md](assets/README.md) 对号入座放进去即可——
> 代码是按目录扫描的，丢进去就能用，不用改任何配置。
>
> 没有素材也能跑起来看界面，只是自习室里没有人。
