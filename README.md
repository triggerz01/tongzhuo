...
> **感知层现在由 Electron 自己拉起**，不用再手动开第二个进程。
> 界面右上角有「开启 / 关闭」和「重连」两个按钮。
> 下面这一节只在你要单独调试感知层时才需要。
...
阈值都在 `renderer/room.js` 的 `LINK` 里，改一行就能调（现在是测试用的短值）：

| 触发条件 | 反应 | 阈值 |
|---|---|---|
| 连续专注满 N 分钟 | 开心夸你 + 点赞 | 1 分钟 |
| 玩手机满 N 分钟 | 失望皱眉 + 叉腰 | 1 分钟 |
| 离席满 N 分钟 | 难过 | 2 分钟 |
| **离席够久**后回到专注 | 你回来了 + 招手 | 需先离席 ≥ 2 分钟 |
| 遮挡镜头 | 疑惑（立即，不用等） | — |
| 专注待机中 | 偶尔犯困/疑惑一下 | 每 4–9 分钟 |

两条设计原则：

- **除了遮挡镜头，状态一变就反应会显得神经质**，其余都要等持续够久
- **短暂离开回来不触发欢迎**。人只是去接了杯水，回来被热烈欢迎一次，
  这句话就廉价了；要离席够久，那句"我等你半天了"才成立

任意两次大反应之间有 45 秒冷却，避免连珠炮。
